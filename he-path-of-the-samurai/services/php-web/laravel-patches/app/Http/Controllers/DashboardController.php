...
class DashboardController extends Controller
{
    ...
    {
        // Получаем CMS блок из БД (безопасно, в контроллере)
        // $cmsBlock = DB::table('cms_blocks')
        //     ->where('slug', 'dashboard_experiment')
        //     ->where('is_active', true)
        //     ->first(['content']);
        
        //$cmsContent = $cmsBlock ? $cmsBlock->content : '';
        $cmsContent = '';
        $b     = $this->base();
        ...
        return view('dashboard', [
            ...
            'cmsContent' => $cmsContent, // Передаем безопасно
        ]);
    }
    ...
    public function jwstFeed(Request $r)
    {
        ...
        foreach ($list as $it) {
            ...
            $items[] = [
                ...
                'obs'      => htmlspecialchars((string)($it['observation_id'] ?? $it['observationId'] ?? ''), ENT_QUOTES, 'UTF-8'),
                'program'  => htmlspecialchars((string)($it['program'] ?? ''), ENT_QUOTES, 'UTF-8'),       
                'suffix'   => htmlspecialchars((string)($it['details']['suffix'] ?? $it['suffix'] ?? ''), ENT_QUOTES, 'UTF-8'),
                'inst'     => array_map(fn($i) => htmlspecialchars($i, ENT_QUOTES, 'UTF-8'), $instList),
                'caption'  => htmlspecialchars(trim(
                    (($it['observation_id'] ?? '') ?: ($it['id'] ?? '')) .
                    ' · P' . ($it['program'] ?? '-') .
                    (($it['details']['suffix'] ?? '') ? ' · ' . $it['details']['suffix'] : '') .
                    ($instList ? ' · ' . implode('/', $instList) : '')
                ), ENT_QUOTES, 'UTF-8'),
                'link'     => htmlspecialchars($loc ?: $url, ENT_QUOTES, 'UTF-8'),
            ];
